...
const { token, mongo, port, admins, support } = require("./config");
const { connect } = require("./services/db");
const { User, Setting, Movie } = require("./models");
const { isSubscribed } = require("./services/channels");
const {
  kb, getUser, gate, welcome, searchMovie, randomMovie, topMovies,
  lastMovies, savedMovies, comments, sendMovie
...
bot.onText(/^\/start$/, async msg => {
  ...
  const premium = user.premiumUntil && user.premiumUntil > new Date();
  if (!premium) {
...
bot.onText(/^\/addmovie$/i, async msg => {
  if (!isAdmin(msg.from.id, admins)) return;
  const u = await getUser(msg); addMovie(bot, msg, u);
});
bot.onText(/^\/addchannel$/i, async msg => {
  if (!isAdmin(msg.from.id, admins)) return;
  const u = await getUser(msg); addChannel(bot, msg, u);
});
bot.onText(/^\/setprice$/i, async msg => {
  if (!isAdmin(msg.from.id, admins)) return;
  const u = await getUser(msg); setPrice(bot, msg, u);
});
...
bot.onText(/^\/emoji$/i, async msg => {
  if (!isAdmin(msg.from.id, admins)) return;
  const u = await getUser(msg); emojiPanel(bot, msg, u);
});
...
bot.onText(/^\/movies$/i, async msg => {
  if (!isAdmin(msg.from.id, admins)) return;
  const rows = await Movie.find().sort({ createdAt: -1 }).limit(50);
...
bot.on("callback_query", async q => {
  ...
  if (data === "check_sub") {
    ...
    const premium = u?.premiumUntil && u.premiumUntil > new Date();
  ...
  if (data === "code") {
    const u = await User.findOne({ telegramId: userId });
    u.step = "movie_search"; await u.save();
  ...
  if (["buy_1","buy_3","buy_12"].includes(data)) {
    ...
    const price = (await Setting.findOne({ key: `price_${plan}` }))?.value || ({1:10000,3:25000,12:75000}[plan]);
    const u = await User.findOne({ telegramId: userId });
    u.temp = { plan, amount: price }; await u.save();
  ...
  if (data.startsWith("verify_")) {
    ...
    const base = (u.premiumUntil && u.premiumUntil > new Date()) ? new Date(u.premiumUntil) : new Date();
    base.setMonth(base.getMonth() + months);
    u.premiumUntil = base; u.temp = {}; await u.save();
    return bot.sendMessage(chatId, "✅ To'lov tasdiqlandi! Premium faollashtirildi.");
  }

  if (data.startsWith("save_")) {
    const code = data.replace("save_", "");
    await User.updateOne({ telegramId: userId }, { $addToSet: { savedMovies: code } });
  ...
  if (data.startsWith("comment_")) {
    ...
    const u = await User.findOne({ telegramId: userId });
    u.step = "comment"; u.temp = { code }; await u.save();
...
bot.on("message", async msg => {
  ...
  if (u.step === "comment") {
    ...
    if (msg.text?.trim()) {
      await require("./models").Comment.create({
        movieCode: code,
        userId: msg.from.id,
        username: msg.from.username || msg.from.first_name,
        text: msg.text.trim()
      });
      await Movie.updateOne({ code }, { $inc: { commentsCount: 1 } });
      u.step = null; u.temp = {}; await u.save();
      return bot.sendMessage(msg.chat.id, "✅ Izoh qabul qilindi.");
    }
  }

  if (u.step === "movie_search" && msg.text?.trim()) {
    u.step = null; await u.save();
    if (!(await gate(bot, msg))) return;
    return searchMovie(bot, msg, msg.text.trim());
  }

  // Oddiy foydalanuvchi istalgan raqam yuborsa kino qidiradi.
  if (msg.text?.trim()) {
    if (!(await gate(bot, msg))) return;
    return searchMovie(bot, msg, msg.text.trim());
  }
